...
void Tuple::serialize(char* dest) const {
    const auto& schema = mRecord.schema();
    auto someNull = !mRecord.schema().allNotNull();
    auto pos = dest;
    ...
    for (decltype(numFields) i = 0; i < numFields; ++i) {
        ...
        if (someNull)
            mRecord.setFieldNull(dest, i, mFields[i].null());
        if (mFields[i].null()) {
            ...
            memset(pos, 0, defaultSize);
            pos += defaultSize;
        } else {
            pos += mFields[i].serialize(pos);
        }
    }
}
